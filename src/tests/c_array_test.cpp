...
FEATURE(CArrayInsert, "Array insert")
	SCENARIO("Insert in middle")
		...
		AND(
				"contain the previous elements, the middle element, plus the next elements")
		for (int i = 0; i < (int) a.size; i++) {
			int *v = static_cast<int*>(CArrayGet(&a, i));
			if (i < middleIndex) {
...
staticbool LessThan2(const void *elem) {
...
FEATURE(CArrayCopy, "Array copy")
	SCENARIO("Copy")
		...
		CA_FOREACH(const int, i, a2)
			SHOULD_INT_EQUAL(*i, _ca_index);CA_FOREACH_END()
		SCENARIO_END
	FEATURE_END
