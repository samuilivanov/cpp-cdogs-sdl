...
// Stubs
int MapNewScan(const char *filename, const bool isArchive, char **title,
		int *numMissions) {
	UNUSED(filename);
	UNUSED(isArchive);
	UNUSED(title);
...
FEATURE(save_and_load, "Save and load")
	SCENARIO("Save and load")
		...
		SHOULD_STR_EQUAL(autosave2.LastMission.Campaign.Path,
				autosave1.LastMission.Campaign.Path);
		AND("their last mission passwords should equal")
		SHOULD_STR_EQUAL(autosave2.LastMission.Password,
...
FEATURE(mission_autosaves, "Mission autosaves")
	...
	SCENARIO("Adding autosave updates last mission")
		...
		SHOULD_STR_EQUAL(autosave.LastMission.Campaign.Path,
