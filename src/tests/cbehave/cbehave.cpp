...
void should_int_gt(int val1, int val2, void *state, const char *file,
		int line) {
	int *_scenario_state = (int*) state;
	if ((val1) <= (val2)) {
		(*_scenario_state) = 1;
...
void should_int_lt(int val1, int val2, void *state, const char *file,
		int line) {
	int *_scenario_state = (int*) state;
	if ((val1) >= (val2)) {
		(*_scenario_state) = 1;
...
void should_int_ge(int val1, int val2, void *state, const char *file,
		int line) {
	int *_scenario_state = (int*) state;
	if ((val1) < (val2)) {
		(*_scenario_state) = 1;
...
void should_int_le(int val1, int val2, void *state, const char *file,
		int line) {
	int *_scenario_state = (int*) state;
	if ((val1) > (val2)) {
		(*_scenario_state) = 1;
...
void should_be_bool(bool actual, bool expected, void *state, const char *file,
	...
	if (actual != expected) {
		...
		printf("\t\t\t%s:%d: Failed: actual[%d] is not a %s value.\n", file,
				line, actual, expected ? "true" : "false");
		resetColor();
	}
}
...
int _cbehave_runner(const char *description, const cbehave_feature *features,
	...
	printf("\tfeatures: [%d/%d]\n",
			state->total_features - state->failed_features,
	...
	printf("\tscenarios: [%d/%d]\n",
			state->total_scenarios - state->failed_scenarios,
...
void* cbehave_mock_obj(const char *fcname, int lineno, const char *fname,
	...
	if (!s) {
		printf(
				"\t[CBEHAVE]: can't find the symbol: <%s> which is being mocked!, %d line in file %s\n",
				fcname, lineno, fname);
		exit(EXIT_FAILURE);
	}

	if (s->always_return_flag)
		return s->value;

	v = get_value(s);
	if (!v) {
		printf(
				"\t[CBEHAVE]: you have not set the value of mock obj <%s>!, %d line in file %s\n",
				fcname, lineno, fname);
		exit(EXIT_FAILURE);
	}
...
static cbehave_symbol_t* lookup_symbol(const char *symbol_name, int obj_type) {
	...
	{
		if (s != NULL) {
			if ((s->obj_type == obj_type) && (!strcmp(s->desc, symbol_name))) {
				return s;
			}
		}
	}
