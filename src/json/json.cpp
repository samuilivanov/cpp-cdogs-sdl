...
rcs_create(size_t length) {
	rcstring *rcs;
	rcs = static_cast<rcstring*>(malloc(sizeof(rcstring))); /* allocates memory for a struct rcstring */
	if (rcs == NULL)
		return NULL;
	...
	rcs->text = static_cast<char*>(malloc((rcs->max + 1) * sizeof(char)));
	if (rcs->text == NULL) {
		free(rcs);
		return NULL;
	}
...
rstring_code rcs_resize(rcstring *rcs, size_t length) {
	...
	temp = static_cast<char*>(realloc(rcs->text, sizeof(char) * (length + 1))); /* length plus '\0' */
	if (temp == NULL) {
		free(rcs);
		return RS_MEMORY;
	}
...
rcs_unwrap(rcstring *rcs) {
	...
	if (rcs->text == NULL)
		...
		out = static_cast<char*>(realloc(rcs->text,
				sizeof(char) * (strlen(rcs->text) + 1)));
	}
	...
	while ((error == JSON_WAITING_FOR_EOF)
		...
		if (fgets(buffer, 1024, file) != NULL) {
			...
			default:
				...
				return static_cast<json_error>(error);
				break;
			}
		} else {
...
json_new_value(const enum json_value_type type) {
	...
	new_object = static_cast<json_t*>(malloc(sizeof(json_t)));
	if (new_object == NULL)
		return NULL;
...
json_new_string(const char *text) {
	...
	new_object->text = static_cast<char*>(malloc(length * sizeof(char)));
...
enum json_error json_tree_to_string(json_t *root, char **text) {
	...
	state1: /* open value */
	{
		...
		case JSON_STRING:
			...
			if (rcs_catcs(output, cursor->text, strlen(cursor->text))
					!= RS_OK) {
				return JSON_MEMORY;
			}
			if (rcs_catc(output, '\"') != RS_OK) {
...
json_unescape(const char *text) {
	char *result = static_cast<char*>(malloc(strlen(text) + 1));
			...
			case 'u': {
				...
				if (unicode < 0x80) {
					...
					if (unicode < 0xD800 || 0xDBFF < unicode) {
						...
						unicode = (high_surrogate << 10) + (low_surrogate)
								+ 0x10000;
...
enum json_error json_parse_document(json_t **root, const char *text) {
	...
	jpi = static_cast<json_parsing_info*>(malloc(
			sizeof(struct json_parsing_info)));
	if (jpi == NULL) {
		return JSON_MEMORY;
	}
	json_jpi_init(jpi);
