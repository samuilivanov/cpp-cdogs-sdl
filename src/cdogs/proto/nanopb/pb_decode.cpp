...
static bool checkreturn buf_read(pb_istream_t *stream, uint8_t *buf,
		size_t count);
static bool checkreturn pb_decode_varint32(pb_istream_t *stream,
		uint32_t *dest);
...
static const pb_decoder_t PB_DECODERS[PB_LTYPES_COUNT] = { &pb_dec_varint,
		&pb_dec_uvarint, &pb_dec_svarint, &pb_dec_fixed32, &pb_dec_fixed64,

		&pb_dec_bytes, &pb_dec_string, &pb_dec_submessage,
		NULL /* extensions */
};
...
static bool checkreturn decode_static_field(pb_istream_t *stream,
	...
	case PB_HTYPE_REPEATED:
		if (wire_type
				== PB_WT_STRING && PB_LTYPE(type) <= PB_LTYPE_LAST_PACKABLE) {
			/* Packed array */
			bool status = true;
			...
			while (substream.bytes_left > 0 && *size < iter->pos->array_size) {
				void *pItem = (uint8_t*) iter->pData
						+ iter->pos->data_size * (*size);
...
static bool checkreturn decode_extension(pb_istream_t *stream, uint32_t tag,
	...
	while (extension != NULL && pos == stream->bytes_left) {
		...
		if (extension->type->decode)
			...
			status = default_extension_decoder(stream, extension, tag,
					wire_type);
...
bool checkreturn pb_decode_noinit(pb_istream_t *stream,
	...
	uint8_t fields_seen[(PB_MAX_REQUIRED_FIELDS + 7) / 8] = { 0, 0, 0, 0, 0, 0,
			0, 0 };
	...
	while (stream->bytes_left) {
		...
		if (PB_HTYPE(iter.pos->type) == PB_HTYPE_REQUIRED
			...
			fields_seen[iter.required_field_index >> 3] |= (uint8_t) (1
					<< (iter.required_field_index & 7));
		}
	...
	/* Check that all required fields were present. */
	{
		...
		if (fields_seen[req_field_count >> 3]
				!= (0xFF >> (8 - (req_field_count & 7))))
			PB_RETURN_ERROR(stream, "missing required field");
	}
