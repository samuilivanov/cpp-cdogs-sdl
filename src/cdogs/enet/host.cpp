...
enet_host_connect(ENetHost *host, const ENetAddress *address,
	...
	currentPeer->channels = (ENetChannel*) enet_malloc(
	...
	command.connect.outgoingPeerID = ENET_HOST_TO_NET_16(
	...
	command.connect.incomingBandwidth = ENET_HOST_TO_NET_32(
			host->incomingBandwidth);
	command.connect.outgoingBandwidth = ENET_HOST_TO_NET_32(
			host->outgoingBandwidth);
	command.connect.packetThrottleInterval = ENET_HOST_TO_NET_32(
			currentPeer->packetThrottleInterval);
	command.connect.packetThrottleAcceleration = ENET_HOST_TO_NET_32(
			currentPeer->packetThrottleAcceleration);
	command.connect.packetThrottleDeceleration = ENET_HOST_TO_NET_32(
...
void enet_host_bandwidth_throttle(ENetHost *host) {
	enet_uint32 timeCurrent = enet_time_get(), elapsedTime = timeCurrent
			- host->bandwidthThrottleEpoch, peersRemaining =
			(enet_uint32) host->connectedPeers, dataTotal = ~0, bandwidth = ~0,
			throttle = 0, bandwidthLimit = 0;
	...
	while (peersRemaining > 0 && needsAdjustment != 0) {
		...
		if (dataTotal <= bandwidth)
			...
			throttle = (bandwidth * ENET_PEER_PACKET_THROTTLE_SCALE)
					/ dataTotal;
		...
		for (peer = host->peers; peer < &host->peers[host->peerCount]; ++peer) {
			...
			if ((throttle * peer->outgoingDataTotal)
					/ ENET_PEER_PACKET_THROTTLE_SCALE <= peerBandwidth)
				continue;
	...
	if (host->recalculateBandwidthLimits) {
		...
		if (bandwidth == 0)
			...
			while (peersRemaining > 0 && needsAdjustment != 0) {
				...
				for (peer = host->peers; peer < &host->peers[host->peerCount];
						++peer) {
					if ((peer->state != ENET_PEER_STATE_CONNECTED
							&& peer->state != ENET_PEER_STATE_DISCONNECT_LATER)
		...
		for (peer = host->peers; peer < &host->peers[host->peerCount]; ++peer) {
			...
			command.header.command = ENET_PROTOCOL_COMMAND_BANDWIDTH_LIMIT
					| ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE;
			command.header.channelID = 0xFF;
			command.bandwidthLimit.outgoingBandwidth = ENET_HOST_TO_NET_32(
					host->outgoingBandwidth);

			if (peer->incomingBandwidthThrottleEpoch == timeCurrent)
				command.bandwidthLimit.incomingBandwidth = ENET_HOST_TO_NET_32(
