...
static size_t commandSizes[ENET_PROTOCOL_COMMAND_COUNT] =
		{ 0, sizeof(ENetProtocolAcknowledge), sizeof(ENetProtocolConnect),
				...
				sizeof(ENetProtocolDisconnect), sizeof(ENetProtocolPing),
...
static int enet_protocol_dispatch_incoming_commands(ENetHost *host,
	...
	while (!enet_list_empty(&host->dispatchQueue)) {
		ENetPeer *peer = (ENetPeer*) enet_list_remove(
				enet_list_begin(&host->dispatchQueue));
...
static void enet_protocol_remove_sent_unreliable_commands(ENetPeer *peer) {
	...
	while (!enet_list_empty(&peer->sentUnreliableCommands)) {
		outgoingCommand = (ENetOutgoingCommand*) enet_list_front(
				&peer->sentUnreliableCommands);
...
static ENetProtocolCommand enet_protocol_remove_sent_reliable_command(
		ENetPeer *peer, enet_uint16 reliableSequenceNumber,
	...
	if (channelID < peer->channelCount) {
		...
		enet_uint16 reliableWindow = reliableSequenceNumber
...
enet_protocol_handle_connect(ENetHost *host, ENetProtocolHeader *header,
	...
	peer->channels = (ENetChannel*) enet_malloc(
			channelCount * sizeof(ENetChannel));
	...
	peer->incomingBandwidth = ENET_NET_TO_HOST_32(
			command->connect.incomingBandwidth);
	peer->outgoingBandwidth = ENET_NET_TO_HOST_32(
			command->connect.outgoingBandwidth);
	peer->packetThrottleInterval = ENET_NET_TO_HOST_32(
	...
	verifyCommand.verifyConnect.outgoingPeerID = ENET_HOST_TO_NET_16(
			peer->incomingPeerID);
	verifyCommand.verifyConnect.incomingSessionID = incomingSessionID;
	verifyCommand.verifyConnect.outgoingSessionID = outgoingSessionID;
	verifyCommand.verifyConnect.mtu = ENET_HOST_TO_NET_32(peer->mtu);
	verifyCommand.verifyConnect.windowSize = ENET_HOST_TO_NET_32(windowSize);
	verifyCommand.verifyConnect.channelCount = ENET_HOST_TO_NET_32(
			channelCount);
	verifyCommand.verifyConnect.incomingBandwidth = ENET_HOST_TO_NET_32(
			host->incomingBandwidth);
	verifyCommand.verifyConnect.outgoingBandwidth = ENET_HOST_TO_NET_32(
			host->outgoingBandwidth);
	verifyCommand.verifyConnect.packetThrottleInterval = ENET_HOST_TO_NET_32(
...
static int enet_protocol_handle_send_unsequenced(ENetHost *host, ENetPeer *peer,
	...
	*currentData += dataLength;
	...
	unsequencedGroup = ENET_NET_TO_HOST_16(
...
static int enet_protocol_handle_send_fragment(ENetHost *host, ENetPeer *peer,
	...
	*currentData += fragmentLength;
	...
	startSequenceNumber = ENET_NET_TO_HOST_16(
	...
	if (startCommand == NULL) {
		...
		startCommand = enet_peer_queue_incoming_command(peer, &hostCommand,
				NULL, totalLength, ENET_PACKET_FLAG_RELIABLE, fragmentCount);
		if (startCommand == NULL)
			return -1;
	}
...
static int enet_protocol_handle_send_unreliable_fragment(ENetHost *host,
		ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData) {
	enet_uint32 fragmentNumber, fragmentCount, fragmentOffset, fragmentLength,
	...
	*currentData += fragmentLength;
	...
	if (startCommand == NULL) {
		startCommand = enet_peer_queue_incoming_command(peer, command, NULL,
				totalLength, ENET_PACKET_FLAG_UNRELIABLE_FRAGMENT,
				fragmentCount);
		if (startCommand == NULL)
			return -1;
	}
...
static int enet_protocol_handle_acknowledge(ENetHost *host, ENetEvent *event,
	...
	receivedSentTime = ENET_NET_TO_HOST_16(
	...
	if (roundTripTime >= peer->roundTripTime) {
		...
		peer->roundTripTimeVariance += (roundTripTime - peer->roundTripTime)
				/ 4;
	} else {
		peer->roundTripTime -= (peer->roundTripTime - roundTripTime) / 8;
		peer->roundTripTimeVariance += (peer->roundTripTime - roundTripTime)
				/ 4;
	}
	...
	receivedReliableSequenceNumber = ENET_NET_TO_HOST_16(
			command->acknowledge.receivedReliableSequenceNumber);

	commandNumber = enet_protocol_remove_sent_reliable_command(peer,
			receivedReliableSequenceNumber, command->header.channelID);
...
static int enet_protocol_handle_verify_connect(ENetHost *host, ENetEvent *event,
	...
	if (channelCount < ENET_PROTOCOL_MINIMUM_CHANNEL_COUNT
			...
			||
			ENET_NET_TO_HOST_32(
					command->verifyConnect.packetThrottleAcceleration)
	...
	peer->outgoingPeerID = ENET_NET_TO_HOST_16(
			command->verifyConnect.outgoingPeerID);
	...
	peer->incomingBandwidth = ENET_NET_TO_HOST_32(
			command->verifyConnect.incomingBandwidth);
	peer->outgoingBandwidth = ENET_NET_TO_HOST_32(
			command->verifyConnect.outgoingBandwidth);
...
static int enet_protocol_handle_incoming_commands(ENetHost *host,
	...
	while (currentData < &host->receivedData[host->receivedDataLength]) {
		...
		command->header.reliableSequenceNumber = ENET_NET_TO_HOST_16(
		...
		case ENET_PROTOCOL_COMMAND_SEND_UNRELIABLE_FRAGMENT:
			if (enet_protocol_handle_send_unreliable_fragment(host, peer,
					command, &currentData))
				goto commandError;
			break;
...
static void enet_protocol_send_acknowledgements(ENetHost *host,
	...
	while (currentAcknowledgement != enet_list_end(&peer->acknowledgements)) {
		if (command
				...
				|| buffer
						>= &host->buffers[sizeof(host->buffers)
								/ sizeof(ENetBuffer)]
		...
		command->acknowledge.receivedReliableSequenceNumber =
				reliableSequenceNumber;
		command->acknowledge.receivedSentTime = ENET_HOST_TO_NET_16(
				acknowledgement->sentTime);
		...
		*command = outgoingCommand->command;
		...
		if (outgoingCommand->packet != NULL) {
			...
			buffer->data = outgoingCommand->packet->data
					+ outgoingCommand->fragmentOffset;
...
static int enet_protocol_send_reliable_outgoing_commands(ENetHost *host,
	...
	while (currentCommand != enet_list_end(&peer->outgoingReliableCommands)) {
			...
			if (!windowWrap && outgoingCommand->sendAttempts < 1
					...
					&& (channel->reliableWindows[(reliableWindow
							...
							|| channel->usedReliableWindows
									& ((((1 << ENET_PEER_FREE_RELIABLE_WINDOWS)
											- 1) << reliableWindow)
											| (((1
		...
		if (enet_list_empty(&peer->sentReliableCommands))
			peer->nextTimeout = host->serviceTime
					+ outgoingCommand->roundTripTimeout;
...
static int enet_protocol_send_outgoing_commands(ENetHost *host,
		ENetEvent *event, int checkForTimeouts) {
	...
	while (host->continueSending)
		for (host->continueSending = 0, currentPeer = host->peers;
			...
			if ((enet_list_empty(&currentPeer->outgoingReliableCommands)
					...
					&& currentPeer->mtu - host->packetSize
           ...
           printf ("peer %u: %f%%+-%f%% packet loss, %u+-%u ms round trip time, %f%% throttle, %u/%u outgoing, %u/%u incoming\n", currentPeer -> incomingPeerID, currentPeer -> packetLoss / (float) ENET_PEER_PACKET_LOSS_SCALE, currentPeer -> packetLossVariance / (float) ENET_PEER_PACKET_LOSS_SCALE, currentPeer -> roundTripTime, currentPeer -> roundTripTimeVariance, currentPeer -> packetThrottle / (float) ENET_PEER_PACKET_THROTTLE_SCALE, enet_list_size (& currentPeer -> outgoingReliableCommands), enet_list_size (& currentPeer -> outgoingUnreliableCommands), currentPeer -> channels != NULL ? enet_list_size (& currentPeer -> channels -> incomingReliableCommands) : 0, currentPeer -> channels != NULL ? enet_list_size (& currentPeer -> channels -> incomingUnreliableCommands) : 0);
				...
				currentPeer->packetLossVariance -=
						currentPeer->packetLossVariance / 4;

				if (packetLoss >= currentPeer->packetLoss) {
					currentPeer->packetLoss += (packetLoss
							- currentPeer->packetLoss) / 8;
					currentPeer->packetLossVariance += (packetLoss
							- currentPeer->packetLoss) / 4;
				} else {
					currentPeer->packetLoss -= (currentPeer->packetLoss
							- packetLoss) / 8;
					currentPeer->packetLossVariance += (currentPeer->packetLoss
							- packetLoss) / 4;
				}
			...
			if (host->headerFlags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME) {
				header->sentTime = ENET_HOST_TO_NET_16(
						host->serviceTime & 0xFFFF);
			...
			if (host->compressor.context != NULL
				...
				size_t originalSize = host->packetSize
						- sizeof(ENetProtocolHeader), compressedSize =
						host->compressor.compress(host->compressor.context,
								&host->buffers[1], host->bufferCount - 1,
								originalSize, host->packetData[1],
								originalSize);
			...
			if (currentPeer->outgoingPeerID < ENET_PROTOCOL_MAXIMUM_PEER_ID)
				host->headerFlags |= currentPeer->outgoingSessionID
						<< ENET_PROTOCOL_HEADER_SESSION_SHIFT;
			header->peerID = ENET_HOST_TO_NET_16(
					currentPeer->outgoingPeerID | host->headerFlags);
			if (host->checksum != NULL) {
				enet_uint32 *checksum =
						(enet_uint32*) &headerData[host->buffers->dataLength];
				*checksum =
						currentPeer->outgoingPeerID
