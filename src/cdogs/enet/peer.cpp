...
void enet_peer_throttle_configure(ENetPeer *peer, enet_uint32 interval,
		enet_uint32 acceleration, enet_uint32 deceleration) {
	...
	command.throttleConfigure.packetThrottleInterval = ENET_HOST_TO_NET_32(
			interval);
	command.throttleConfigure.packetThrottleAcceleration = ENET_HOST_TO_NET_32(
			acceleration);
	command.throttleConfigure.packetThrottleDeceleration = ENET_HOST_TO_NET_32(
			deceleration);
...
int enet_peer_send(ENetPeer *peer, enet_uint8 channelID, ENetPacket *packet) {
	...
	if (packet->dataLength > fragmentLength) {
		enet_uint32 fragmentCount = (packet->dataLength + fragmentLength - 1)
				/ fragmentLength, fragmentNumber, fragmentOffset;
		...
		for (fragmentNumber = 0, fragmentOffset = 0;
			...
			fragment = (ENetOutgoingCommand*) enet_malloc(
			...
			fragment->command.sendFragment.startSequenceNumber =
					startSequenceNumber;
			fragment->command.sendFragment.dataLength = ENET_HOST_TO_NET_16(
					fragmentLength);
			fragment->command.sendFragment.fragmentCount = ENET_HOST_TO_NET_32(
					fragmentCount);
			fragment->command.sendFragment.fragmentNumber = ENET_HOST_TO_NET_32(
					fragmentNumber);
			fragment->command.sendFragment.totalLength = ENET_HOST_TO_NET_32(
					packet->dataLength);
			fragment->command.sendFragment.fragmentOffset = ENET_NET_TO_HOST_32(
					fragmentOffset);
	...
	if ((packet->flags
		...
		command.sendUnsequenced.dataLength = ENET_HOST_TO_NET_16(
				packet->dataLength);
	} else if (packet->flags & ENET_PACKET_FLAG_RELIABLE
			|| channel->outgoingUnreliableSequenceNumber >= 0xFFFF) {
		command.header.command = ENET_PROTOCOL_COMMAND_SEND_RELIABLE
				| ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE;
		command.sendReliable.dataLength = ENET_HOST_TO_NET_16(
				packet->dataLength);
	} else {
		command.header.command = ENET_PROTOCOL_COMMAND_SEND_UNRELIABLE;
		command.sendUnreliable.dataLength = ENET_HOST_TO_NET_16(
				packet->dataLength);
	}
...
static void enet_peer_reset_outgoing_commands(ENetList *queue) {
	...
	while (!enet_list_empty(queue)) {
		outgoingCommand = (ENetOutgoingCommand*) enet_list_remove(
...
static void enet_peer_remove_incoming_commands(ENetList *queue,
		ENetListIterator startCommand, ENetListIterator endCommand) {
...
void enet_peer_reset_queues(ENetPeer *peer) {
	...
	if (peer->channels != NULL && peer->channelCount > 0) {
		for (channel = peer->channels;
			...
			enet_peer_reset_incoming_commands(
...
enet_peer_queue_acknowledgement(ENetPeer *peer, const ENetProtocol *command,
	...
	if (command->header.channelID < peer->channelCount) {
		...
		if (reliableWindow
				...
				&& reliableWindow
						<= currentWindow + ENET_PEER_FREE_RELIABLE_WINDOWS)
			return NULL;
	}

	acknowledgement = (ENetAcknowledgement*) enet_malloc(
...
void enet_peer_setup_outgoing_command(ENetPeer *peer,
	...
	peer->outgoingDataTotal += enet_protocol_command_size(
			outgoingCommand->command.header.command)
...
enet_peer_queue_outgoing_command(ENetPeer *peer, const ENetProtocol *command,
		ENetPacket *packet, enet_uint32 offset, enet_uint16 length) {
...
void enet_peer_dispatch_incoming_unreliable_commands(ENetPeer *peer,
	...
	for (droppedCommand = startCommand = currentCommand = enet_list_begin(
			&channel->incomingUnreliableCommands);
			currentCommand
		...
		if (incomingCommand->reliableSequenceNumber
			...
			if (startCommand != currentCommand) {
				enet_list_move(enet_list_end(&peer->dispatchedCommands),
						startCommand, enet_list_previous(currentCommand));
	...
	for (currentCommand = enet_list_begin(&channel->incomingReliableCommands);
		...
		if (incomingCommand->fragmentsRemaining > 0
				|| incomingCommand->reliableSequenceNumber
						!= (enet_uint16) (channel->incomingReliableSequenceNumber
								+ 1))
			break;
...
enet_peer_queue_incoming_command(ENetPeer *peer, const ENetProtocol *command,
	...
	case ENET_PROTOCOL_COMMAND_SEND_UNRELIABLE_FRAGMENT:
		unreliableSequenceNumber = ENET_NET_TO_HOST_16(
				command->sendUnreliable.unreliableSequenceNumber);
	...
	incomingCommand = (ENetIncomingCommand*) enet_malloc(
			sizeof(ENetIncomingCommand));
	...
	discardCommand: if (fragmentCount > 0)
		goto notifyError;
