...
ENetSocket enet_socket_create(ENetSocketType type) {
	return socket(PF_INET,
			type == ENET_SOCKET_TYPE_DATAGRAM ? SOCK_DGRAM : SOCK_STREAM, 0);
}
...
int enet_socket_set_option(ENetSocket socket, ENetSocketOption option,
	...
	case ENET_SOCKOPT_RCVBUF:
		result = setsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char*) &value,
				sizeof(int));
		break;

	case ENET_SOCKOPT_SNDBUF:
		result = setsockopt(socket, SOL_SOCKET, SO_SNDBUF, (char*) &value,
				sizeof(int));
		break;
...
int enet_socket_connect(ENetSocket socket, const ENetAddress *address) {
	...
	result = connect(socket, (struct sockaddr*) &sin,
