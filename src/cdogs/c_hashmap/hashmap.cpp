...
map_t hashmap_new(void) {
	map_t m = static_cast<map_t>(malloc(sizeof(struct hashmap_map)));
	if (!m)
		goto err;
...
static int copy_key(any_t data, any_t key) {
	...
	return hashmap_put(hData->dst, static_cast<char*>(key), copy);
}
...
static unsigned int hashmap_hash_int(const map_t m, const char *keystring) {
	...
	unsigned long key = crc32((const unsigned char*) keystring,
			strlen(keystring));
...
int hashmap_put(map_t m, const char *key, any_t value) {
	...
	m->data[index].key = static_cast<char*>(malloc(strlen(key) + 1));
 ...
 */
static int iterate(map_t m, PFany f, any_t item,
		int (*func)(hashmap_element, PFany, any_t));
...
static int hashmap_return_first(any_t data, any_t item) {
	any_t *arg = static_cast<any_t*>(data);
	*arg = item;
	return MAP_FULL;
}
...
void hashmap_destroy(map_t in, void (*callback)(any_t)) {
	...
	// support function/data pointer conversion
	if (callback) {
...
static int hashmap_destroy_item_callback(any_t a, any_t b) {
	void (**callback)(any_t) = static_cast<void (**)(any_t)>(a);
	if (callback != NULL) {
		(*callback)(b);
	}
	return MAP_OK;
